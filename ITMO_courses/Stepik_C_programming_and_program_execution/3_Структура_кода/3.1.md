...
void interpret(struct vm_state *state) {
  while (state->ip->opcode != BC_STOP) {
    switch (state->ip->opcode) {
      ...
      case BC_IADD: {
        struct maybe_int64 op1 = stack_pop(&(state->data_stack));
        struct maybe_int64 op2 = stack_pop(&(state->data_stack));
        if (op1.valid && op2.valid) {
          stack_push(&(state->data_stack), op1.value + op2.value);
        }
        break;
      }
      default: break;
...
void interpret(struct vm_state* state, ins_interpreter * const (actions)[]) {
    for (; state->ip;) {
        ...
        if (state->data_stack.count < ins_descr->stack_min || 
            (ins_descr->stack_delta + state->data_stack.count) > state->data_stack.data.size) {
            printf("Stack %s\n", state->data_stack.count < ins_descr->stack_min ? "underflow" : "overflow");
            break;
        }
